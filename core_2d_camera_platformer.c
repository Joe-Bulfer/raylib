    ...
    //}
		...
		//if click, get x and y of click, 
		    ...
		    if (IsMouseButtonDown(0)) {
			    clickedX = mapGrid.x;
			    clickedY = mapGrid.y;
			    //clickedX = (int)(mapGrid.x / 20) * 20;
			    //clickedY = (int)(mapGrid.y / 20) * 20;
            		    //DrawText(("IsMouseButtonDown at %i %i",clickedX, clickedY), 20, 20, 10, BLACK);
			    //printf("IsMouseButtonDown at %i %i",clickedX, clickedY);
			    
			    if (blockCount < maxBlocks) {
				clickedPositions[blockCount] = mapGrid;  // Store the click position
				blockCount++;  // Increment the counter
		}
            }
			
		if (clickedX != -1 && clickedY != -1) {  // Check if the click occurred
            		DrawRectangle(clickedX, clickedY, 10, 10, BLUE);  // Draw the rectangle at the click position
        }
